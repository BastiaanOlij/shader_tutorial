...
vec2 calculate_wave_uv_offset(in float time, vec2 source_uvs, vec2 time_multiplier, vec2 waves_scale, inout vec2 wave_uv_offset) {
	wave_uv_offset.x += sin(time * time_multiplier.x + (source_uvs.x + source_uvs.y) * waves_scale.x);
	wave_uv_offset.y += cos(time * time_multiplier.y + (source_uvs.x + source_uvs.y) * waves_scale.y);
